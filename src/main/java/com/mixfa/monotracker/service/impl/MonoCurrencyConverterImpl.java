...
public class MonoCurrencyConverterImpl implements MonoCurrencyConverter {
    ...
    @Override
    public long convert(long amount, int currencyCodeA, int currencyCodeB) throws Exception {
        if (currencyCodeA == currencyCodeB) return amount;
        var currencyRates = monoApi.getCurrencies();
        var rate = Arrays.stream(currencyRates)
                .filter(r ->
                        r.currencyCodeA() == currencyCodeA &&
                                r.currencyCodeB() == currencyCodeB)
                .findFirst().orElse(
                        Arrays.stream(currencyRates)
                                .filter(r ->
                                        r.currencyCodeA() == currencyCodeB &&
                                                r.currencyCodeB() == currencyCodeA)
